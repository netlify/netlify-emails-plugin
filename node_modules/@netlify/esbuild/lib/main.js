    ...
    for (let item of plugins) {
      ...
      try {
        ...
        let promise = setup({
          ...
          onDynamicImport(options, callback2) {
            ...
            let filter = getFlag(options, keys2, "filter", mustBeRegExp) || /.*/;
            let namespace = getFlag(options, keys2, "namespace", mustBeString);
            checkForInvalidFlags(options, keys2, `in onDynamicImport() call for plugin ${JSON.stringify(name)}`);
            let id = nextCallbackID++;
            onDynamicImportCallbacks[id] = { name, callback: callback2, note: registeredNote };
    ...
    const callback = async (request) => {
      switch (request.command) {
        ...
        case "dynamicImport": {
          ...
          for (let id of request.ids) {
            try {
              ({ name, callback: callback2, note } = onDynamicImportCallbacks[id]);
              let result = await callback2({
              ...
              if (result != null) {
                if (typeof result !== "object")
                  throw new Error(`Expected onDynamicImport() callback in plugin ${JSON.stringify(name)} to return an object`);
                let keys = {};
                let pluginName = getFlag(result, keys, "pluginName", mustBeString);
                let contents = getFlag(result, keys, "contents", mustBeStringOrUint8Array);
                let pluginData = getFlag(result, keys, "pluginData", canBeAnything);
